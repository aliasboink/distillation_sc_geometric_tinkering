...
register_dataset(S3DIS) 

import torch_geometric
from torchdistill.datasets.transform import TRANSFORM_CLASS_DICT
# This removes the "torchvision" dictionary and modifies it to only have the 
# torch_geometric transforms. Not a good solution.
TRANSFORM_CLASS_DICT = dict()
TRANSFORM_CLASS_DICT.update(torch_geometric.transforms.__dict__)

from custom_eval.dense_gcn_eval import geometric_segmentation_accuracy
import custom_additions.compatibility
# ===============================================
...
def load_model(model_config, device):
    if 'segmentation_model' not in model_config:
        return load_segmentation_model(model_config, device, strict=True)
...
def evaluate(model_wo_ddp, data_loader, device, device_ids, distributed, num_classes, no_dp_eval=False,
    ...
    metric_logger = MetricLogger(delimiter='  ')
    for sample_batch, targets, supp_dict in metric_logger.log_every(data_loader, log_freq, header):
        if isinstance(sample_batch, torch.Tensor):
            sample_batch = sample_batch.to(device)
        ...
        I, U = geometric_segmentation_accuracy(pred, targets, 13)
